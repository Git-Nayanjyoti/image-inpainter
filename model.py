...
class RestorationModel(pl.LightningModule):
    def __init__(self, lr: float, num_layers: int = 10, num_features: int = 64):
        ...
        self.model = RestorationAutoEncoder(
            num_layers=self.hparams.num_layers, num_features=self.hparams.num_features
        )
    ...
    def training_step(self, batch, batch_idx):
        ...
        loss = F.binary_cross_entropy_with_logits(input=outputs, target=cln_ims)

        self.log_dict(
            dict(train_loss=loss), prog_bar=True, on_epoch=True, on_step=False
        )
        return loss
    ...
    def configure_optimizers(self):
        ...
        opt = optim.AdamW(
            self.model.parameters(), lr=self.hparams.lr, weight_decay=1e-05
        )
...
if __name__ == "__main__":
    ...
    model = RestorationModel.load_from_checkpoint(state)
